...
if torch.cuda.is_available():
    ...
    def synthesis_waveglow(model, text, iter, alpha=1.0, beta=1.0, gamma=1.0):
        batch_text = []
        batch_src_pos = []
        ...
        with torch.no_grad():
            mel, _, _, _ = model.forward(batch_text.to(device), batch_src_pos.to(device), max_src_pos,
                                         e_control=alpha, d_control=beta, p_control=gamma)
        return mel[iter].unsqueeze(0).contiguous().transpose(1, 2)

    model.to(device)
    data_list = get_data()
    for energy in [0.8, 1., 1.3]:
        for i, phn in tqdm(enumerate(data_list)):
            mel = synthesis_waveglow(model, phn, i, alpha=energy)

            os.makedirs("results", exist_ok=True)

            waveglow.inference.inference(
                mel, WaveGlow,
                f"results/s={energy}_{i}_waveglow.wav"
            )

    for i, phn in tqdm(enumerate(data_list)):
        mel = synthesis_waveglow(model, phn, i, alpha=0.8, beta=0.8, gamma=0.8)

        os.makedirs("results", exist_ok=True)

        waveglow.inference.inference(
            mel, WaveGlow,
            f"results/s_all={0.8}_{i}_waveglow.wav"
        )

    for i, phn in tqdm(enumerate(data_list)):
        mel = synthesis_waveglow(model, phn, i, alpha=1.2, beta=1.2, gamma=1.2)

        os.makedirs("results", exist_ok=True)

        waveglow.inference.inference(
            mel, WaveGlow,
            f"results/s_all={1.2}_{i}_waveglow.wav"
        )

    for i, phn in tqdm(enumerate(data_list)):
        mel = synthesis_waveglow(model, phn, i, alpha=1, beta=1, gamma=1)

        os.makedirs("results", exist_ok=True)

        waveglow.inference.inference(
            mel, WaveGlow,
            f"results/base_configs={i}_waveglow.wav"
        )
